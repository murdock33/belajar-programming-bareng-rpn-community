  ...
  for (let i = 0; i < listBarang.length; i++) {
    ...
    for (let j = 0; j < shoppers.length; j++) {
      if (
        ...
        shoppers[j].amount <= productObject.leftOver
      ) {
        productObject.shoppers.push(shoppers[j].name);

        productObject.leftOver -= shoppers[j].amount;
        productObject.totalProfit += shoppers[j].amount * listBarang[i][1];
      }
    }
