...
function missingNum(arr) {
  ...
  // find the max and mini number
  let maxNumber = array[0];
  let miniNumber = array[0];

  for (let i = 0; i < array.length; i++) {
    if (array[i] > maxNumber) {
      maxNumber = array[i];
    } else if (array[i] < miniNumber) {
      miniNumber = array[i];
    }
  }
  ...
  // result
  let result = [];
  for (let i = miniNumber; i <= maxNumber; i++) {
    // Periksa apakah nilai i tidak ada di dalam array hasil
    if (!array.includes(i)) {
      result.push(i);
    }
  }
